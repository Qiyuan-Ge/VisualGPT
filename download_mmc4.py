...
Example Usage:
    python download_images.py --input_jsonl ./data_core/docs_no_face_shard_0_v3.jsonl
        OR
    python download_images.py --input_shards "https://storage.googleapis.com/ai2-jackh-mmc4-public/data_core/docs_no_face_shard_{0..23098}_v3.jsonl.zip" --output_image_dir mmc4_images/
"""
...
def parse_args():
    ...
    parser.add_argument('--num_process', type=int, default=32, help='Number of processes in the pool can be larger than cores')
...
def main():
    ...
    if args.input_jsonl is not None:
        ...
        for idx in range(len(doc_shards)):
            ...
            pattern = r"shard_(\d+)"
            match = re.search(pattern, doc_shards[idx])
            if match:
                shard_number = int(match.group(1))
                print(f"The shard number is: {shard_number}")
            else:
                print("No shard number found in the string.")
            print("Downloading zip for shard", shard_number)
            
            try:
                fname = os.path.join(doc_folder, doc_shards[idx].split("/")[-1])
                urllib.request.urlretrieve(doc_shards[idx], fname)
                ...
                with zipfile.ZipFile(fname, "r") as zip_file:
                    ...
                    shard_folder = args.output_image_dir + "/" + str(shard_number)
